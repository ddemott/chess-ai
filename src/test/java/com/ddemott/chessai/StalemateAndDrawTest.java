...
public class StalemateAndDrawTest {
	...
	void testBasicStalemate() {
		try {
			...
			// Set up a basic stalemate: Black king on h8, White queen on g6, White king on
			// f7
			...
			assertFalse(engine.getGameState().getBoard().isKingInCheck("Black"),
					"King should not be in check in stalemate");
		} catch (Exception e) {
	...
	void testStalemateWithMultiplePieces() {
		try {
			...
			engine.getGameState().getBoard().setPieceAt("b6", new Queen("White", "b6")); // Controls b8 and diagonal
																							// squares; change to ensure
																							// it's not checking a8
			// Bishop moved so it doesn't give check to a8; not necessary for this stalemate
			engine.getGameState().getBoard().setPieceAt("c5", new Bishop("White", "c5")); // Additional control (not
																							// checking a8)
	...
	void testStalemateTrap() {
		try {
			...
			engine.getGameState().getBoard().setPieceAt("h8", new King("Black", "h8"));
			engine.getGameState().getBoard().setPieceAt("f7", new King("White", "f7"));
			engine.getGameState().getBoard().setPieceAt("h6", new Queen("White", "h6")); // Queen can move to create
																							// stalemate
	...
	void testInsufficientMaterial() {
		try {
			...
			assertTrue(isInsufficientMaterial(engine),
					"King and bishop vs king and knight should be insufficient material");
			...
			assertTrue(isInsufficientMaterial(engine),
					"King and two knights vs king should be considered insufficient material");
	...
	/**
	 * Helper method to determine if the position has insufficient material for
	 * checkmate This is a simplified implementation for testing
	 */
